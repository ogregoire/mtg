...
│
├── PlayerSelector
│   ├── PlayerSelector.Anyone               ← NEW (enum, ANY — always-true)
...
└── (NEW nested arms)
    ├── ObjectPropertySelector.Anything                              ← enum, ANY
    ├── ObjectPropertySelector.AllOf(List<ObjectPropertySelector>)
    ├── ObjectPropertySelector.AnythingOf(List<ObjectPropertySelector>)
...
public sealed interface ObjectPropertySelector permits
        ...
        ObjectPropertySelector.AnythingOf,
    ...
    enum Any implements ObjectPropertySelector { ANY }   // always-true predicate
...
## PlayerSelector — nested Any + Enchanted

```java
public non-sealed interface PlayerSelector extends Selector {
    enum Any implements PlayerSelector { ANY }
...
| "any graveyard" (owner slot) | `Graveyard(PlayerSelector.Anyone.ANY, Card(Anything.ANYTHING))` |
| "~" | `SelfSelector.SELF` |

## Files

### Create (new files) / Restructure
- `selector/ObjectPropertySelector.java` — **NEW**. Sealed interface with nested `Any`, `AllOf`, `AnyOf`, `Not`, `Enchanted`, `Equipped`, `Fortified`. Permits the existing standalone axes (listed above) plus the nested arms.
...
- `PlayerSelector.java` — add nested `Any` enum and nested `Enchanted` record. Sealing decision: see PlayerSelector section.
