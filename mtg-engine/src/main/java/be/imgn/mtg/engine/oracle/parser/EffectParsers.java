...
final class EffectParsers {
    ...
    /// comma.
    private static final Parser<Duration.ForAsLongAs> AS_LONG_AS_PREFIX = phrase("As long as")
    ...
    /// "The damage can't be prevented." — back-reference to the
    /// damage dealt by the preceding effect (Pinpoint Avalanche).
    ...
    /// power."); Tree of Perdition ("Exchange target opponent's
    /// life total with this creature's toughness.").
    ...
    static final Parser<Effect.AddCardType> ADD_CARD_TYPE = sequence(
                    ARE_SUBJECT,
                    MtgParsers.andList(CARD_TYPE).followedBy(phrase("in addition to [its|their] other types")),
    ...
    private static final Parser<Effect> BASE_EFFECT = Parser.<Effect>anyOf(
            ...
            CHOOSE_TYPE, // must precede CHOOSE
    ...
    /// "The next time [subject] would [event] [this turn]?, [replacement]
    /// instead." — next-occurrence replacement (Words of Worship: "The
    /// next time you would draw a card this turn, you gain 5 life
    ...
    private static final Parser<Effect.ForEach> FOR_EACH_EFFECT = sequence(
            anyOf(
                            // "For each of <selector>," — per-target iteration
                            // over an already-bounded subject (Doppelgang:
                            // "For each of X target permanents, create X
                            // tokens …"). Semantically equivalent to plain
                            // "For each <selector>" for the ForEach record.
                            phrase("For each of").then(SELECTOR),
                            phrase("For each").then(SELECTOR))
                    .followedBy(","),
            BASE_EFFECT
                    .<Effect>map(e -> e)
    ...
    /// per-distinct-property loop (Bloom Tender: "For each color
    /// among permanents you control, add one mana of that color.").
    /// Distinct from [#FOR_EACH_EFFECT]: iteration is over values
    /// of a property, not over matching objects.
    private static final Parser<Effect.ForEachAmong> FOR_EACH_AMONG_EFFECT = sequence(
            phrase("For each")
                    .then(anyOf(
    ...
    private static final Parser<Subject.PlayerRef> FOR_EACH_PLAYER_REF = anyOf(
            ...
            // "any number of opponents" — chooser-picks-subset form
            // (Windgrace's Judgment: "For any number of opponents,
    ...
    public static final Parser<Effect> EFFECT = Parser.<Effect>anyOf(
                    ...
                    FOR_EACH_AMONG_EFFECT, // must precede FOR_EACH_EFFECT ("color among" is specific)
