...
public sealed interface Trait {
    ...
    boolean test(GameObject object);
    ...
    record CardType(Type type) implements Trait {
        ...
        public boolean test(GameObject object) {
            return object instanceof TypedObject t && t.types().contains(type);
        }
    }
    ...
    record ObjectColor(Color color) implements Trait {
        @Override
        public boolean test(GameObject object) {
            return object instanceof TypedObject t && t.colors().contains(color);
        }
    }
    ...
    record ObjectSupertype(Supertype supertype) implements Trait {
        @Override
        public boolean test(GameObject object) {
            return object instanceof TypedObject t && t.supertypes().contains(supertype);
        }
    }
