...
/// refines it into structured sub-types. The {@link Kind} distinguishes
/// the two semantic flavors so consumers don't have to parse English:
/// {@link Kind#IF} means the effect resolves only when the predicate
/// holds; {@link Kind#UNLESS} is the negation — the effect is countered
/// unless the predicate is satisfied (e.g., by paying a cost).
public record Condition(Kind kind, String text) {

    /// Convenience: an {@code if}-style condition.
    public static Condition ifCondition(String text) {
        return new Condition(Kind.IF, text);
    }

    /// Convenience: an {@code unless}-style condition (the negation).
